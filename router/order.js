    ...
    try {
        ...
        if (keys.includes('filterBy')) {
            if (req.body.filterBy[0].fieldName == 'mode') {
                if (req.body.filterBy[0].value.length > 0 && !(req.body.filterBy[0].value[0] == 'LATEST_ORDER')) {
...
// router.post('/order-count', verifyToken, async (req, res) => {
            ...
            where order_id='${id}'`
                ...
                db.query(query, async (err, result) => {
                    ...
                    if (err)
                        ...
                        return resMessage(res, true, "update succesfully")
                })
            }
            else
                return resMessage(res, false, `fileds should not be empty `)
        }
        else {
...
//     try {
//         let requiredKeys = ["product_name", "name", "phone", "email", "status", "zip_code", "city", "state", "remark", "quantity", "address", "payment_mode"]
//         let keysExistValue = keysExist(requiredKeys, req, res)
//         if (keysExistValue.status) {
//             let body = { ...req.body }
...
//                 return resMessage(res, false, `Field should not be empty ${emptyValues}`)
//             } else {
//                 let query = `INSERT INTO order_detail (product_name, name, phone, email, status, zip_code, city, state, remark, quantity, address, mode) VALUES (?)`
//                 db.query(query, [dataToInsert], async (err, result) => {
//                     if (err)
//                         return resMessage(res, false, "something went wrong")
//                     else
//                         return resMessage(res, true, "Add succesfully")
//                 })
...
router.put('/order-update/:id', verifyToken, async (req, res) => {
    ...
    try {
        ...
        let requiredKeys = ["product_name", "name", "phone", "email", "status", "zip_code", "city", "state", "remark", "quantity", "address", "mode"]
        let keysExistValue = keysExist(requiredKeys, req, res)
        if (keysExistValue.status) {
            let body = { ...req.body }
            let bodyValues = Object.values(body)
...
router.get('/today-product', verifyToken, async (req, res) => {
    ...
    try {
        ...
        let query = `SELECT product_name,idtag,
