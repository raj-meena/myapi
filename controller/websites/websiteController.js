...
exports.create_website = async (req, res) => {
  try {
    ...
    if (keysExistValue.status) {
      ...
      try {
        var xmlData = convert.xml2json(req.body.sitemap_url, {
          compact: true,
          spaces: 4,
        });
        data_insert = [
          {
            ...
            sitemap_url: req.body.sitemap_url
              ? req.body.sitemap_url
              : req.body.sitemap_url,
        ...
        db.query(
          `select url from websites where url='${url}'`,
          function (err, result) {
            if (err) {
            } else {
              if (!result.length) {
...
//we are fecthing data with particular id from table
